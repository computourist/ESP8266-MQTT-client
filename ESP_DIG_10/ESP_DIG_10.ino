...
//
	...
	#define VERSION "WDIG V1.0"						// this value can be queried as device 3
	...
	if (length == 0) {error = 2;}					// no payload sent
		...
		if (DID==5) {								// ACK
			if (readAction) {
				...
				if (setAck ==0) sprintf(buff_msg, "OFF");
				if (setAck ==1) sprintf(buff_msg, "ON");
				pubMQTT(buff_topic, buff_msg);
				error = 0;
		...
		if (DID==6) {								// toggle / timer mode selection
			if (readAction) {
				...
				if (toggleOnButton ==0) sprintf(buff_msg, "OFF");
				if (toggleOnButton ==1) sprintf(buff_msg, "ON");
				pubMQTT(buff_topic, buff_msg);
				error = 0;
