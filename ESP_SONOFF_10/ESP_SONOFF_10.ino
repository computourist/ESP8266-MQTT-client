...
#define VERSION "Sonoff V1.0"					// this value can be queried as device 3
...
void mqttSubs(char* topic, byte* payload, unsigned int length) {	// receive and handle MQTT messages
	...
	void reconnect() {								// reconnect to mqtt broker
		...
		if (!client.loop()) {
			mqttCon = true;							// LED off means high voltage
#ifdef DEBUG
			Serial.print("Connect to MQTT broker...");
#endif
			if (client.connect("ESP_GW")) { 
				mqttCon = false;					// LED on means low voltage
			...
			digitalWrite(MQCON, mqttCon);			// adjust MQTT link indicator
			}
		}
